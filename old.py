        ...
        try:
            ...
            with client:
                ...
                if message == "ping":
                    ...
                    my_position = state["position"][my_index]
                    ...
                    state = req["state"]
                    my_index = state["current"]
                    my_position = state["positions"][my_index]
                    #
                    #-------------
                    #
                    tile = state["tile"]
                    ...
                    print("moves = ",moves)
                    #
                    if len(moves) < 2:  # if seq contains 0 or 1 element
                        random_move = moves[0] if moves else None  # select the first element, or None if seq is empty
                    elif moves == []:
                        random_move = my_position
                    else:
                        random_move = random.choice(moves)
                    #
                    # print(state["board"])
                    print("mypos", state["positions"][my_index])
                    #
...
def successors(index, state):
    ...
    for direction in ["N", "S", "E", "W"]:
        if board[current_pos][direction]:
            coords = add(index2coords(current_pos), DIRECTIONS[direction]["coords"])
            if isCoordsValid(*coords):
                adj_tile = board[coords2index(*coords)]
                opposite_dir = DIRECTIONS[direction]["opposite"]
                if adj_tile[opposite_dir]:
...
#         dirName = DIRECTIONS[dir]["name"]  # changes (-1, 0) to "N" 
#         opposite = DIRECTIONS[dirName]["opposite"]
#         print("Coords:", coords)
#         print("IsCoordsValid:", isCoordsValid(*coords))
#         print("Board Opposite Value:", board[coords2index(*coords)][opposite])
#         # breakpoint()
#         if isCoordsValid(*coords):
#             print('OKKKKK')
